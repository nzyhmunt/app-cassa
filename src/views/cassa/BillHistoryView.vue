<template>
  <div class="flex-1 flex flex-col bg-gray-100/80 overflow-y-auto p-4 md:p-8 relative min-h-0">
    <div class="max-w-4xl mx-auto w-full">
      ...
      <!-- Summary bar -->
      <div v-if="orderStore.closedBills.length > 0" class="grid grid-cols-3 gap-3 mb-6">
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 text-center">
          ...
          <p class="text-2xl font-black text-gray-800">{{ orderStore.closedBills.length }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-emerald-200 shadow-sm p-4 text-center">
          ...
          <p class="text-2xl font-black text-emerald-700">{{ configStore.config.ui.currency }}{{ totalRevenue.toFixed(2) }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 text-center">
          <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Media Conto</p>
          <p class="text-2xl font-black text-gray-800">{{ configStore.config.ui.currency }}{{ averageBill.toFixed(2) }}</p>
        </div>
      </div>

      <!-- Empty state -->
      <div v-if="orderStore.closedBills.length === 0" class="text-center py-20">
      ...
      <div v-else class="space-y-3">
        <CassaBillCard
          v-for="bill in orderStore.closedBills"
          :key="billKey(bill)"
          :bill="bill"
        />
      </div>
...
import CassaBillCard from '../../components/CassaBillCard.vue';

const configStore = useConfigStore();
const orderStore = useOrderStore();

const totalRevenue = computed(() =>
  orderStore.closedBills.reduce((sum, b) => sum + b.totalPaid, 0)
);

const averageBill = computed(() =>
  orderStore.closedBills.length > 0 ? totalRevenue.value / orderStore.closedBills.length : 0
);
</script>
