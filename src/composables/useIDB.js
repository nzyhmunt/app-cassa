 ...
 *
 * To add a new version (e.g. v6):
 *   1. Increment DB_VERSION to 6.
 *   2. Add a new `if (oldVersion < 6) { ... }` block inside the `upgrade()` callback.
    ...
    async upgrade(db, oldVersion, _newVersion, tx) {
      ...
      if (
        ...
        try {
          ...
          console.warn('[useIDB] Failed to migrate legacy local settings from app_settings to local_settings during v5 upgrade.', {
            dbName,
            oldVersion,
            newVersion: DB_VERSION,
            error,
