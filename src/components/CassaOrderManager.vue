  ...
  <div class="flex-1 flex overflow-hidden relative">
    ...
    <aside :class="['w-full md:w-[380px] lg:w-[450px] bg-white border-r border-gray-200 flex flex-col shadow-lg z-10 h-full shrink-0', selectedOrder ? 'hidden md:flex' : 'flex']">
      ...
      <div class="flex p-2 gap-1.5 bg-gray-50 border-b border-gray-200 shrink-0">
        <button @click="changeTab('pending')" aria-label="In Attesa" :class="activeTab === 'pending' ? 'bg-amber-100 text-amber-800 border-amber-200 font-bold' : 'bg-white text-gray-500 border-gray-200 hover:bg-gray-100'" class="flex-1 py-1.5 md:py-2 px-1 rounded-xl border transition-all flex items-center justify-center gap-1.5 shadow-sm">
          <div class="relative shrink-0">
            <Bell class="size-4 md:size-5" />
            <span v-if="orderStore.pendingCount > 0" class="absolute -top-1.5 -right-2 bg-red-500 text-white text-[9px] font-bold size-4 flex items-center justify-center rounded-full border border-white">{{ orderStore.pendingCount }}</span>
          </div>
          <span class="text-[9px] md:text-[10px] uppercase tracking-wider hidden sm:inline">In Attesa</span>
        </button>
        <button @click="changeTab('accepted')" aria-label="In Cucina" :class="activeTab === 'accepted' ? 'bg-teal-100 text-teal-800 border-teal-200 font-bold' : 'bg-white text-gray-500 border-gray-200 hover:bg-gray-100'" class="flex-1 py-1.5 md:py-2 px-1 rounded-xl border transition-all flex items-center justify-center gap-1.5 shadow-sm">
          <div class="relative shrink-0">
            <ChefHat class="size-4 md:size-5" />
            <span v-if="acceptedCount > 0" class="absolute -top-1.5 -right-2 bg-teal-600 text-white text-[9px] font-bold size-4 flex items-center justify-center rounded-full border border-white">{{ acceptedCount }}</span>
          </div>
          <span class="text-[9px] md:text-[10px] uppercase tracking-wider hidden sm:inline">In Cucina</span>
        </button>
        <button @click="changeTab('history')" aria-label="Chiusi" :class="activeTab === 'history' ? 'bg-gray-200 text-gray-800 border-gray-300 font-bold' : 'bg-white text-gray-500 border-gray-200 hover:bg-gray-100'" class="flex-1 py-1.5 md:py-2 px-1 rounded-xl border transition-all flex items-center justify-center gap-1.5 shadow-sm">
          <div class="relative shrink-0">
        ...
        <div class="bg-white border-b border-gray-200 p-3 md:p-5 shrink-0 shadow-sm z-10 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 md:gap-4">
          ...
          <div class="flex gap-2 w-full sm:w-auto mt-2 sm:mt-0 items-center justify-end">
            ...
            <template v-if="selectedOrder.status === 'pending'">
              <button @click="requestDeleteOrder" class="flex-1 sm:flex-none px-3 py-2.5 md:py-3 bg-white text-red-600 border border-red-200 hover:bg-red-50 rounded-xl font-bold flex items-center justify-center gap-1.5 active:scale-95 transition-colors">
                <Trash2 class="size-5" /> <span class="hidden sm:inline text-xs md:text-sm">Elimina</span>
              </button>
              <button @click="acceptAndPrint(selectedOrder)" class="flex-[2] sm:flex-none px-4 py-2.5 md:py-3 theme-bg text-white shadow-md rounded-xl font-bold flex items-center justify-center gap-1.5 active:scale-95 hover:opacity-90 transition-opacity" title="Invia comanda in cucina">
            ...
            <template v-else-if="selectedOrder.status === 'accepted'">
              <div class="flex gap-2 w-full items-center">
                ...
                <button @click="forceDeliver(selectedOrder)" class="px-3 py-2.5 md:py-3 bg-gray-500 hover:bg-gray-600 text-white shadow-md rounded-xl font-bold flex items-center justify-center gap-1.5 active:scale-95 transition-colors text-xs shrink-0" title="Segna consegnata (override cucina)">
            ...
            <template v-else-if="selectedOrder.status === 'delivered'">
              <span class="w-full text-center px-4 py-2.5 md:py-3 bg-gray-100 text-gray-600 border border-gray-200 rounded-xl font-bold flex items-center justify-center gap-2">
        ...
        <div class="pt-4 border-t border-gray-100">
          ...
          <div v-if="configStore.config.ui.allowCustomVariants" class="flex gap-2 mb-3">
            ...
            <div class="relative w-24 shrink-0">
              ...
              <NumericInput v-model="noteModal.modPrice" min="0" step="0.50" placeholder="0.00" :prefix="configStore.config.ui.currency" class="w-full pl-7 pr-2 py-3 bg-gray-100 border border-gray-200 rounded-xl focus:bg-white theme-ring transition-all text-gray-800 font-medium text-sm" />
            </div>
  ...
  <div v-if="showAddMenuModal" class="fixed inset-0 z-[80] bg-black/80 backdrop-blur-sm flex items-end md:items-center justify-center p-0 md:p-4">
    <div class="bg-white rounded-t-3xl md:rounded-3xl shadow-2xl w-full max-w-6xl h-[95dvh] md:h-[85dvh] flex flex-col overflow-hidden relative">
      ...
      <MenuCartPanel
        ...
        :cart="tempCart"
        :qty-map="menuPanelQtyMap"
        :currency="configStore.config.ui.currency"
        cart-title="Carrello Preparazione"
        empty-cart-message="Tocca i piatti nel menu per prepararli qui, poi inseriscili."
        confirm-label="Inserisci nella Comanda"
        total-label="Totale Aggiunte:"
        ...
        @update-qty="updateTempCartQty"
        @edit-cart-item="openCartNoteModal"
        @remove-mod="removeModFromCart"
        @confirm="confirmAndPushCart"
      />
    </div>
...
const filteredOrders = computed(() => {
  if (activeTab.value === 'history')
    return orderStore.orders.filter(o => (o.status === 'completed' || o.status === 'rejected') && !o.isDirectEntry);
...
function removeModFromCart(cartIdx, modIdx) {
  ...
  if (cartItem && cartItem.modifiers) {
    cartItem.modifiers.splice(modIdx, 1);
  }
}
...
const menuPanelQtyMap = computed(() => {
  const map = {};
  for (const [k, v] of orderQtyMap.value) map[k] = (map[k] || 0) + v;
  for (const [k, v] of cartQtyMap.value) map[k] = (map[k] || 0) + v;
  return map;
});
