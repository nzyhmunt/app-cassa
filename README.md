...
- `online_only`:
  - operazioni inviate direttamente a Directus (senza coda IDB `sync_queue`)
  - pull incrementale canonico di riallineamento
  - trigger intelligenti su interazione (cambio view, focus/visibility tab, ritorno online) con refresh route-aware mirato per collection
...
│   ├── useOnlineOnlyInteractionRefresh.js ← Pull di riallineamento semantico in `online_only` (route/focus/visibility/online) con throttle e collection scope route-aware
...
- Selettore **Modalità operativa**:
  ...
  - `online_only`:
    - push diretto su Directus (senza coda locale)
    - pull incrementale canonico di riconciliazione
    - sugli eventi di interazione viene privilegiato un refresh mirato per route/collection (fallback al pull completo)
